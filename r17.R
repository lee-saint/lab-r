...
# R에서 학습용으로 제공하는 mpg 데이터프레임
str(mpg)

# 배기량(displ)과 고속도로 연비(hwy)와의 관계
...
str(economics)
# 시간(date)에 따른 개인 저축률(psavert)의 변화 - 선그래프
g <- ggplot(data = economics, mapping = aes(x = date, y = psavert)) + geom_line()
ggplotly(g)


# 시계열 데이터에 대한 더 많은 기능을 갖고 있는 패키지
install.packages("dygraphs")
library(xts)
library(dygraphs)

# xts: 시계열 그래프를 그려주는, dygraphs 패키지에서 사용하는 데이터타입
...
eco_unemploy <- xts(x = economics$unemploy / 1000, order.by = economics$date)
eco_uempmed <- xts(x = economics$uempmed, order.by = economics$date)
data2 <- cbind(eco_uempmed, eco_unemploy)
head(data2)
dygraph(data2)
