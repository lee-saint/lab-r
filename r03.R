...
# 수학 점수를 저장하는 벡터
math <- c(50, 60, 25, 100)
# 영어점수, 수학점수로 이루어진 표(테이블) - 데이터프레임
scores <- data.frame(english, math)

scores  # 데이터프레임의 모든 데이터 출력
scores$math
mean(scores$math)
mean(scores)
...
setwd("디렉토리(폴더) 경로")  # set working dirctory: 현재 작업 디렉토리 변경
...
# ex) 현재 작업 디렉토리가 C:/dev/lab-r인 경우에
...
fruits <- data.frame(product, price, sales)
fruits
