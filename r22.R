...
mydata <- read.csv(file = "mlwr/usedcars.csv", stringsAsFactors = F)
# 데이터프레임의 구조(structure) 확인
str(mydata)
# 데이터 일부 확인
head(mydata)
tail(mydata)
...
library(ggplot2)
ggplot(data = mydata, mapping = aes(y = price)) + geom_boxplot()
ggplot(data = mydata, mapping = aes(y = mileage)) + geom_boxplot() + ylab("주행 거리") + ggtitle("주행거리 상자 그림")

# 히스토그램(histogram): 연속된 수치 구간을 일정한 간격으로 나눠서 그 구간에 해당하는 데이터의 개수(빈도수) 막대로 그린 그래프
...
ggplot(data = mydata, mapping = aes(x = mileage)) + geom_histogram(bins = 8, color = "black", fill = 'white')

# 분산(varince),표준편차(standard deviation)
var(mydata$price)
sd(mydata$price)
var(mydata$mileage)
sd(mydata$mil)

# 정규분표: 평균을 중심으로 양쪽이 대칭인 종 모양의 분포
...
# 2개 이상의 변수들 간의 상관관계 -> 산점도(산포도) 그래프
# 중고차의 주행거리(mileage)가 가격(price)에 영향?
plot(x = mydata$mileage, y = mydata$price, main = "주행거리 vs 가격", xlab = "주행거리(mile)", ylab = "가격(USD)")

ggplot(data = mydata, mapping = aes(x = mileage, y = price, color = model)) + geom_point() + xlab("주행거리(mile)") + ylab("가격(USD)") + ggtitle("주행거리 vs 가격")


# 이원교차표(two-way cross table)
# 두 개의 명목 변수 간의 관계를 파악
install.packages("gmodels")
library(gmodels)

# mydata 데이터프레임에 새로운 변수를 추가
